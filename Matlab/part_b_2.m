...
dN_prey = @(N_prey, N_pred) R_o_prey*(1 - N_prey/K).*N_prey - (y*N_prey./(1+y*h*N_prey)).*N_prey.*N_pred;
dN_pred = @(N_prey, N_pred) e*(y*N_prey./(1+y*h*N_prey)).*N_prey.*N_pred -R_o_pred.*N_pred;

% Define initial conditions
% (three cases)
N_prey_0 = [200 500 1000];
N_pred_0 = [50  100  80];
